"""Batch brand-pass for DecoAI Home Decor videos, dataset 1205.

Reads from 1205/<lang>/*.mp4, writes to 1205/_branded/<lang>/*.mp4.
Language split — each language stays in its own folder for per-geo Meta Ads campaigns.

Run:
    python -u scripts/batch_brand_pass_decoai_1205.py [--workers N] [--limit N] [--seed-base N]
"""
...
WORK_ROOT = Path(r"D:/temp/brandpass")  # use D: drive (43GB free) instead of C: (98% full)
BRAND_TITLE = "DecoAI"
BRAND_SUB   = "Redesign Any Room in Seconds"

# Language folders to process. Excludes _super_saiyan / _branded / Unknown / raw dirs.
LANG_DIRS = [
    "Deutsch", "English", "Español", "Français", "Italiano",
    "Polski", "Tiếng Việt", "Türkçe", "Русский", "日本語",
]
...
def main():
    ...
    jobs = []
    for lang_name in LANG_DIRS:
        lang_dir = SRC_ROOT / lang_name
        if not lang_dir.is_dir():
            continue
        out_lang = DST_ROOT / lang_name
        for mp4 in sorted(lang_dir.glob("*.mp4")):
